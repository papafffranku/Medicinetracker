...
class invitefriends : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        getarr.setOnClickListener{
            db.collection("alarms").whereEqualTo("puid", Firebase.auth.currentUser?.uid.toString()).get()
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        Log.d("Main", "${document.data}")
                        val timeofmed = document["alarmtext"] as List<*>?
                        Log.d("Main", "Time: ${timeofmed}")
